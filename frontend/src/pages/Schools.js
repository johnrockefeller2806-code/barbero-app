...
export const Schools = () => {
  ...
  // Show Paywall if user is not PLUS (disabled)
  if (showPaywall) {
    return (
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24">
          <div className="flex items-center justify-between">
            <div>
              ...
              <motion.p 
                className="text-emerald-200 text-lg max-w-2xl"
                initial={{ opacity: 0, x: -30 }}
                animate={{ opacity: 1, x: 0 }}
            ...
            <CardContent className="p-4">
              <div className="flex flex-col md:flex-row gap-4">
                <div className="relative flex-1">
                  ...
                  <Input
                    placeholder={language === 'pt' ? 'Buscar escolas...' : language === 'es' ? 'Buscar escuelas...' : 'Search schools...'}
                    value={searchTerm}
                ...
                <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  <Button variant="outline" className="h-12 px-6 rounded-xl gap-2">
                    ...
                    {language === 'pt' ? 'Filtros' : language === 'es' ? 'Filtros' : 'Filters'}
                  </Button>
                </motion.div>
              </div>
            </CardContent>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 py-12">
        <StaggerChildren className="grid grid-cols-1 md:grid-cols-2 gap-6" staggerDelay={0.1}>
          {filteredSchools.map((school, index) => {
            ...
            return (
              <StaggerItem key={school.id} direction="up">
                <Link to={`/schools/${school.id}`}>
                  <HoverCard scale={1.02}>
                    <Card 
                      className="group overflow-hidden border-slate-100 hover:shadow-2xl transition-all duration-300"
                    ...
                    >
                      <div className="relative h-48 overflow-hidden">
                        ...
                        <div className="absolute top-4 left-4 flex gap-2">
                          {school.accreditation?.slice(0, 2).map((acc, i) => (
                            ...
                            >
                              <Badge className="bg-white/90 text-emerald-800 text-xs">
                                {acc}
                              </Badge>
                            </motion.div>
                          ))}
                      ...
                      <CardContent className="p-6">
                        ...
                        <p className="text-slate-500 text-sm mb-4 line-clamp-2">
                          {language === 'pt' ? school.description : school.description_en || school.description}
                        </p>
                        <div className="flex items-center justify-between">
        ...
        {filteredSchools.length === 0 && (
          <FadeIn direction="up">
            <div className="text-center py-16" data-testid="no-schools">
              ...
              >
                {language === 'pt' ? 'Nenhuma escola encontrada.' : language === 'es' ? 'Ninguna escuela encontrada.' : 'No schools found.'}
              </motion.p>
            </div>
          </FadeIn>
        )}
